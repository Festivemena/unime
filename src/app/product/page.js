...
const page = () => {
        ...
        <div className=" w-full">
          <div className="w-full flex-1">
            {items.map((item, index) => (
              <div onClick={()=>{router.back()}} key={index} className="">
                <div className='w-full flex px-3'>
                <Image src={item.image} alt={item.name} width={22} height={22} className="object-cover rounded-full" />
                <div className="mt-1 text-[12px] ml-3 font-semibold">{item.name}</div>
                </div>
  ...
  return (
    <div className='w-full h-full'>
        ...
        <div className='w-full text-[14px] font-medium px-3'>Collar shirt available in different color as seen in the picture above. Thick material and long lasting.</div>
      ...
      <div className='bg-white justify-between flex rounded-2xl w-[90%] ml-[5%] h-[70%]'>
        <div className=' flex justify-between ml-[5%] mt-3 w-[90%]'>
          ...
          <div className='bg-[#FEC222] h-12 py-2 text-xl text-white font-bold rounded-lg text-center w-2/3'>Call Seller</div>
      </div>
      </div>
      </div>
    </div>
